...
function OpenFridgeMenu()
    ...
    local elements = {
      {label = _U('get_objectf'), value = 'get_stock'},
      {label = _U('put_objectf'), value = 'put_stock'}
    }
...
function OpenRecettesMenu()

  local elements = {
    {label = _U('recettes_b'), value = 'recettes_b'},
    {label = _U('recettes_f'), value = 'recettes_f'}
  }
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'recettem',
    {
      title    = _U('recettem'),
      align    = 'top-left',
      elements = elements,
    ...
    function(data, menu)
      ...
      if data.current.value == 'recettes_b' then
        ESX.ShowNotification(_U('recettes_bb1'))
        ESX.ShowNotification(_U('recettes_bb2'))
        ESX.ShowNotification(_U('recettes_bb3'))
        ESX.ShowNotification(_U('recettes_bb4'))
        ESX.ShowNotification(_U('recettes_bb5'))
        ESX.ShowNotification(_U('recettes_bb6'))
      end

      if data.current.value == 'recettes_f' then
        ESX.ShowNotification(_U('recettes_ff1'))
        ESX.ShowNotification(_U('recettes_ff2'))
      end
...
function OpenPreparationsNMenu()
  ...
  ESX.UI.Menu.Open(
        ...
        function(data, menu)
          ...
          if data.current.value == 'cuttomate' then
            TriggerServerEvent('esx_burgerjob:ingredientgBurger', 'cuttomate')
            animsAction({ lib = "mini@drinking", anim = "shots_barman_b" })
          end

          if data.current.value == 'lavesalade' then
            TriggerServerEvent('esx_burgerjob:ingredientgBurger', 'lavesalade')
            animsAction({ lib = "mini@drinking", anim = "shots_barman_b" })
          end

          if data.current.value == 'cuiresteak' then
            TriggerServerEvent('esx_burgerjob:ingredientgBurger', 'cuiresteak')
            animsAction({ lib = "mini@drinking", anim = "shots_barman_b" })
          end

          if data.current.value == 'friteuse' then
            TriggerServerEvent('esx_burgerjob:ingredientgBurger', 'friteuse')
            animsAction({ lib = "mini@drinking", anim = "shots_barman_b" })
          end
...
function OpenPreparationsmMenu()
  ...
  ESX.UI.Menu.Open(
        ...
        function(data, menu)
          ...
          if data.current.value == 'burger' then
            TriggerServerEvent('esx_burgerjob:craftingBurger', 'burger')
            animsAction({ lib = "mini@drinking", anim = "shots_barman_b" })
          end
  
          if data.current.value == 'fritesba' then
            TriggerServerEvent('esx_burgerjob:craftingBurger', 'fritesba')
            animsAction({ lib = "mini@drinking", anim = "shots_barman_b" })
          end
...
function OpenShopMenu(zone)
    ...
    for i=1, #Config.Zones[zone].Items, 1 do
        ...
        table.insert(elements, {
            label     = item.label .. '<span style="color:#03adfc;">$' .. item.price .. ' </span>',
...
AddEventHandler('esx_burgerjob:hasEnteredMarker', function(zone)
    ...
    if zone == 'Recettes' then
      CurrentAction     = 'recettem'
      CurrentActionMsg  = _U('recettemm')
      CurrentActionData = {}
    end

    if zone == 'PreparationsN' then
      CurrentAction     = 'menu_ingredient'
      CurrentActionMsg  = _U('menu_ingredientm')
      CurrentActionData = {}
    end

    if zone == 'Preparationsm' then
      CurrentAction     = 'menu_crafting'
      CurrentActionMsg  = _U('menu_craftingm')
      CurrentActionData = {}
    end
  ...
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlJustReleased(0,  Keys['E']) and IsJobTrue() then
        ...
        if CurrentAction == 'recettem' then
          OpenRecettesMenu()
        end

        if CurrentAction == 'menu_ingredient' then
          OpenPreparationsNMenu()
        end

        if CurrentAction == 'menu_crafting' then
          OpenPreparationsmMenu()
        end
...
Citizen.CreateThread(function()
  ...
  ped = CreatePed("PED_TYPE_CIVFEMALE", "s_m_y_chef_01", -1183.52, -906.26, 12.45, 305.70, true, true)
